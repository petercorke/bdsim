...
class Gain(FunctionBlock):
    ...
    def __init__(
        ...
        **blockargs: Any,
        ...
        self.K: int | float | np.ndarray[tuple[Any, ...], np.dtype[Any]] = K
...
class Function(FunctionBlock):
    ...
    def output(self, t: float, inputs: list[Any], x: Any) -> list[Any]:
        ...
        if callable(self.func):
            # single function
            try:
                val = self.func(*inputs, *self.args, **self.kwargs)
            except TypeError:
                raise RuntimeError(
            ...
            for f in self.func:
                try:
                    val = f(*inputs, *self.args, **self.kwargs)
                except TypeError:
                    raise RuntimeError(
                        "Function invocation failed, check number of arguments"
                    ) from None
        ...
        if simstate is not None:
            if self.time:
                ...
                if self.xpts[-1] is np.inf:
                    self.xpts[-1] = simstate.T
                assert self.xpts[-1] >= simstate.T, "interpolation not defined for t>T"
