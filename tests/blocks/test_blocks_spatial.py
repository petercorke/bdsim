...
class SpatialMathBlockTest(unittest.TestCase):
    ...
    def test_pose_postmul(self):
        ...
        B = SE3.Rz(np.pi / 2)
        nt.assert_array_almost_equal(block.test_output(B)[0], B * A)
    ...
    def test_pose_premul(self):
        ...
        block = Pose_premul(A)
        B = SE3.Rz(np.pi / 2)
        nt.assert_array_almost_equal(block.test_output(B)[0], A * B)
    ...
    def test_pose_inverse(self):
        ...
        nt.assert_array_almost_equal(block.test_output(A)[0], A.inv())
    ...
    def test_vector_transform(self):
        ...
        B = np.r_[1, 2, 3]
        nt.assert_array_almost_equal(block.test_output(A, B)[0], A * B)
