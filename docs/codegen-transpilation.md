...
`encoder → pwm`, `encoder → direction`. Every generated file opens with
a header (generation timestamp, a "don't hand-edit" note) and gives each
block its own full-width `TYPE (instance name)` separator — real output,
not a mockup:

```cpp
//============================================================
// Generated by bdsim.codegen on 2026-09-20 16:56:52
...
void encoder_output(
    ...
    encoder_outports& outports
);

//============================================================
//  ANALOGOUT (pwm_magnitude)
//============================================================
...
void pwm_magnitude_output(
    ...
    pwm_magnitude_outports& outports
);
```
...
Every block is called the same *way* —
`pwm_magnitude_output(t, g_x, pwm_magnitude_self_inst,
...
`bd.plan` (bdsim's own dataflow schedule) deliberately excludes every
sink/graphics-classed block, since bdsim's own Python engine calls them
separately (`BlockDiagram.step()`, "at the end of every integration
interval"), not through the dataflow plan at all. Codegen mirrors that:
after the schedule loop, a trailing
`/****** I/O sink outputs (not in bd.plan) ******/` block calls every
I/O sink's `{name}_output()` once, after every wire that could feed it
...
void bdsim_tick(double t) {
    ...
    pwm_magnitude_inports_inst._0 = encoder_outports_inst._0;  // pwm_magnitude[0] <-- encoder[0]
    motor_direction_inports_inst._0 = encoder_outports_inst._0;  // motor_direction[0] <-- encoder[0]

    /****** I/O sink outputs (not in bd.plan) *******/
    pwm_magnitude_output(t, g_x, pwm_magnitude_self_inst, pwm_magnitude_inports_inst, pwm_magnitude_outports_inst);
    motor_direction_output(t, g_x, motor_direction_self_inst, motor_direction_inports_inst, motor_direction_outports_inst);
}
```
